...
rm "$folder_name"/live.txtls
    ...
    echo "Total $(wc -l < "$folder_name"/newurls.txtl) live websites found" | notify -silent -provider telegram
    ...
    cat "$folder_name"/newurls.txtls | nuclei -t /home/max/.local/nuclei-templates/ -severity critical,medium,high,low -rl 30 -c 2  -o "$folder_name/bugs.txt" | notify -silent -provider telegram 
